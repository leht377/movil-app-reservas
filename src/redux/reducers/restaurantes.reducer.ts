...
const restaurantes = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(
      uploadFotoInstalacionRestaurante.pending,
      (state, action) => {
        state.status_aceptar_reserva = Status.LOADING;
      }
    );
    builder.addCase(
      uploadFotoInstalacionRestaurante.rejected,
      (state, action) => {
        // state.status_aceptar_reserva = Status.FAILED
        if (action.payload instanceof AxiosError)
          state.error = action.payload.response?.data?.error;
    ...
    builder.addCase(
      uploadFotoInstalacionRestaurante.fulfilled,
      (state, action) => {
        // state.status_aceptar_reserva = Status.SUCCEEDED
        state.restaurante = action.payload;
      }
    ...
    builder.addCase(
      deleteFotoInstalacionRestaurante.pending,
      (state, action) => {
        // state.status_aceptar_reserva = Status.LOADING
      }
    );
    builder.addCase(
      deleteFotoInstalacionRestaurante.rejected,
      (state, action) => {
        // state.status_aceptar_reserva = Status.FAILED
        if (action.payload instanceof AxiosError)
          state.error = action.payload.response?.data?.error;
    ...
    builder.addCase(
      deleteFotoInstalacionRestaurante.fulfilled,
      (state, action) => {
        // state.status_aceptar_reserva = Status.SUCCEEDED
        state.restaurante = action.payload;
      }
