...
const useEliminarFavorito = () => {
  const { cliente } = useAppSelector((state) => state.cliente)
  const { usuario } = useAppSelector((state) => state.usuario)
  const dispatch = useAppDispatch()
  const eliminarFavorito = async (restauranteid: string): Promise<ClienteEntity> => {
    try {
      ...
      const clienteResponse = await clienteServices.eliminarRestauranteFavorito(dto)
...
export default useEliminarFavorito
